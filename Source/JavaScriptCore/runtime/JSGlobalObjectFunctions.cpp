...
EncodedJSValue JSC_HOST_CALL globalFuncReadAccelerometer(ExecState* exec)
{
	...
	JSValue jsX = jsNumber(x);
	JSValue jsY = jsNumber(y);
	JSValue jsZ = jsNumber(z);

	JSArray* jsArray = constructEmptyArray(exec);
	jsArray->push(exec, jsX);
	jsArray->push(exec, jsY);
	jsArray->push(exec, jsZ);

	JSValue value(jsArray);
	return JSValue::encode(value);
